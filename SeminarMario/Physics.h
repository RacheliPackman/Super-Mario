...
public:
	...
	// check collision bottom with other entitie's physics.
	virtual bool checkCollisionBottom(std::shared_ptr<IPhysicsComponent> const& other) const = 0;
...
public:
	...
	virtual bool checkCollisionBottom(std::shared_ptr<IPhysicsComponent> const& other) const override;
...
public:
	...
	virtual cv::Point const& getVelocity() const override;
	friend bool checkPixelLevelCollisionBottom(IPhysicsComponent const* first, IPhysicsComponentPtr const& other);
};
...
public:
	...
	JumpPhysics(int horizontalVelocity, int initialVerticalVelocity, int gravity, int bottom);
	virtual void reset(cv::Point const& tl);
	virtual bool update(cv::Mat const& collisionMask);
	virtual cv::Mat const& getCollisionMask() const override;
	virtual bool checkCollision(IPhysicsComponentPtr const& other) const;
